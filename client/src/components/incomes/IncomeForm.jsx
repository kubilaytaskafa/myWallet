...
import { useCreateIncomeMutation, useUpdateIncomeMutation } from '../../store/api/incomesApi'
import { useGetResidentsQuery } from '../../store/api/residentsApi'

const CATEGORIES = ['Maaş', 'Serbest Meslek', 'Kira Geliri', 'Yatırım', 'Hediye', 'Diğer']

function IncomeForm({ editItem, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editItem) await updateIncome({ id: editItem.id, ...payload }).unwrap()
      else          await createIncome(payload).unwrap()
      onClose()
  ...
  return (
    <form id="income-form" onSubmit={handleSubmit}>
      ...
      <div className="mb-3">
        ...
        <input id="income-title" name="title" type="text" className="glass-form-control" value={form.title} onChange={handleChange} required />
      </div>

      <div className="row mb-3">
        <div className="col-md-6">
          <label className="form-label-navy">Miktar (₺) *</label>
          <input id="income-amount" name="amount" type="number" step="0.01" min="0" className="glass-form-control" value={form.amount} onChange={handleChange} required />
        </div>
        <div className="col-md-6">
          <label className="form-label-navy">Tarih *</label>
          <input id="income-date" name="date" type="date" className="glass-form-control" value={form.date} onChange={handleChange} required />
        </div>
      </div>
      ...
      <div className="row mb-3">
        <div className="col-md-6">
          ...
          <select id="income-category" name="category" className="glass-select" value={form.category} onChange={handleChange}>
            {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div className="col-md-6">
          <label className="form-label-navy">Kişi</label>
          <select id="income-resident" name="resident_id" className="glass-select" value={form.resident_id} onChange={handleChange}>
      ...
      <div className="mb-3">
        ...
        <textarea id="income-desc" name="description" className="glass-form-control" rows={2} value={form.description} onChange={handleChange} />
      </div>

      <div className="d-flex gap-2">
        <button id="income-submit-btn" type="submit" className="btn btn-forest flex-grow-1" disabled={isLoading}>
          {isLoading ? <span className="spinner-border spinner-border-sm me-1" /> : <i className="fas fa-plus me-1" />}
          {editItem ? 'Güncelle' : 'Gelir Ekle'}
        </button>
